...
!$omp parallel do schedule(dynamic) private(j,i,alon)
  do j=1,nlon
...
!$omp parallel do schedule(dynamic) private(j,i,dxx,dxxi,dxxh)
  do i=1,nlat
...
!$omp parallel do schedule(dynamic) private(j,i,jr,ir)
  do j=1,ny
     jr=ny+1-j
     do i=1,nx
        ir=nx+1-i
...
!$omp parallel do schedule(dynamic) private(j,i,gxa)
  do i=1,nx
     do j=1,ny
        gxa=xbh_b(i,j)
        if(gxa < xa_a(1))then
...
!$omp parallel do schedule(dynamic) private(i,j,gya)
...
!$omp &  private(j,i,rlat,diff,rlon,xr,yr,zr,xu,yu,zu,uval,ewval,nsval)
  do j=1,ny+1
     do i=1,nx
...
!$omp parallel do schedule(dynamic) &
!$omp &  private(j,i,rlat,rlon,diff,xr,yr,zr,xv,yv,zv,vval,ewval,nsval)
  do j=1,ny
     do i=1,nx+1
...
!!!!!!!!!! output in reverse E-W, N-S and reversed i,j !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     nybp=nyb+1
     nxbp=nxb+1
!$omp parallel do schedule(dynamic) private(j,i,ir,ijr,jr)
     do i=1,nyb
        ir=nybp-i
        ijr=(ir-1)*nxb
        do j=1,nxb
...
!$omp parallel do schedule(dynamic) private(j,i,ijr)
     do i=1,nyb
        ijr=(i-1)*nxb
        do j=1,nxb
...
!$omp parallel do schedule(dynamic)&
!$omp &  private(j,i,x,y,z,xt,yt,zt,xtt,ytt,ztt)
  do j=1,ny
     do i=1,nx
!   1.  compute x,y,z from rlon_in, rlat_in
...
!$$$ end documentation block
  ...
  integer(i_kind) i,j
!$omp parallel do schedule(dynamic) &
!$omp & private(j,i,xtt,ytt,ztt,xt,yt,zt,x,y,z)
  do j=1,ny
     do i=1,nx
