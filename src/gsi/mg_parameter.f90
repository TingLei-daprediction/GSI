...
!                                                                      !
!  For 4 multigrid generations the following must be satisfied:        !
!                                                                      !
!    If   mod(nxm,8)>0  then    mod(im0,8)=0                           !
!    If   mod(mym,8)>0  then    mod(jm0,8)=0                           !
!                                                                      !
...
use berror, only: mg_ampl01,mg_ampl02,mg_ampl03
use berror, only: nxm_filt,mym_filt,im_filt,jm_filt
use berror, only: hx_filt,hy_filt,hz_filt,p_filt
use berror, only: lm_filt,lmh_filt,lmf_filt
use berror, only: nm0_filt,mm0_filt
!TEST
...
!STD logical:: mgbf_line
!integer(i_kind):: nxPE,nyPE
!STD logical:: lquart,lhelm
...
!
! Total Number of grid points on the analysis grid 
!
    nm0 = nm0_filt
    mm0 = mm0_filt

!
! Number of grid points on the analysis grid in each PE
!
