...
use mg_mppstuff, only: mpi_comm_comp,mpi_comm_work
...
real(r_kind), dimension(lm_all,1-hx:im+hx,1-hy:jm+hy),intent(inout):: Warray
...
integer(i_kind) sHandle(4),rHandle(4),ISTAT(MPI_STATUS_SIZE)
integer(i_kind) iaerr,ierr,ndata,i,j,k
integer(i_kind) isend,irecv,nebpe
...
!   endif
!TEST
!     write(300+mype,*) 'mype,Warray (1,1,1)=',mype,Warray(1,1,1)
!            call finishMPI
!TEST
...
integer(i_kind) sHandle(4),rHandle(4),ISTAT(MPI_STATUS_SIZE)
integer(i_kind) iaerr,ierr,ndata,i,j,L
integer(i_kind) isend,irecv,nebpe
...
integer(i_kind) iaerr,ierr,iderr,i,j
integer(i_kind) isend,irecv,nebpe
integer(i_kind) ndatax
logical l_sidesend
integer(i_kind) g_ind,g,k
