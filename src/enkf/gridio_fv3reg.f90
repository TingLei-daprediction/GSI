...
subroutine writegriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,vargrid,no_inflate_flag)
    ...
    implicit none
    include 'netcdf.inc'    !cltorg  think, is this line needed 
...
!cltorg      fv3filename=trim(adjustl(filename))//"_dynvartracer"
 ...
 !to be implemented in the future
    ...
    integer :: i,j,ii, k,ianal,nn,ntile,nn_tile0, nb,nanal,ne,iret
...
!    prse_ind = getindex(vars3d, 'prse') ! pressure
    ...
    allocate(qsatworkvar3d(nx_res,ny_res,nlevs))
    allocate(tvworkvar3d(nx_res,ny_res,nlevs))
!clt the pnc part is  following as much as possible from gridio_gfs 
  ...
  ipe=0
  do isub=1,numproc_io_member ! to make sure each of the first ntasks_io PE
...
!              if (nproc .eq. 0)                                               &
      ...
      if (v_ind > 0) then
       ...
       do k=1,nlevs
         call mpi_gatherv(locvworkvar3d(1:nxloc,1:nyloc,k), recvcounts2d(iope+1), mpi_real4, workvar2d, recvcounts2d, displs2d,&
                     mpi_real4, 0,iocomms(mem_pe(nproc)) ,iret)
...
!           if (nproc .eq. 0)                                               &
     ...
     if (w_ind > 0) then
        ...
        do k=1,nlevs
           call mpi_gatherv(locworkvar3d(1:nxloc,1:nyloc,k), recvcounts2d(iope+1), mpi_real4, workvar3d(:,:,k), recvcounts2d, displs2d,&
                       mpi_real4, 0,iocomms(mem_pe(nproc)) ,iret)
        enddo
        if(iope==0) then
          do k=1,nlevs
...
!              if (nproc .eq. 0)                                               &
    ...
    if (tv_ind > 0.or.tsen_ind) then
       ...
       enddo
       do k=1,nlevs
          call mpi_gatherv(locqworkvar3d(1:nxloc,1:nyloc,k), recvcounts2d(iope+1), mpi_real4, qworkvar3d(:,:,k), recvcounts2d, displs2d,&
                      mpi_real4, 0,iocomms(mem_pe(nproc)) ,iret)
       enddo
...
!              if (nproc .eq. 0)   then                                           
    ...
    if (oz_ind > 0) then
        ...
        call read_fv3_restart_data3d(varstrname,fv3filename,file_id,locworkvar3d)
        do k=1,nlevs
          call mpi_gatherv(locworkvar3d(1:nxloc,1:nyloc,k), recvcounts2d(iope+1), mpi_real4, workvar3d(1:,:,k), recvcounts2d, displs2d,&
                    mpi_real4, 0,iocomms(mem_pe(nproc)) ,iret)
        enddo
        if(iope ==0 ) then
          do k=1,nlevs
   ...
   !cltorg      fv3filename=trim(adjustl(filename))//"_dynvartracer"
       ...
       !u-component
         ...
         if (u_ind > 0) then
            ...
            if (my_neb(1)>-1) then ! I have a northern neighboring subdomain
...
!            call mpi_wait(ihandle(1),istat,iret)
         ...
         if (v_ind > 0) then
            ...
            if(iope == 0) then
             ...
             if (my_neb(2)>-1) then ! I have a eastern neighboring subdomain
         ...
         if (tv_ind > 0.or.tsen_ind>0 ) then
            ...
            call read_fv3_restart_data3d(varstrname,fv3filename,file_id,loctsenworkvar3d)
            do k=1,nlevs
               call mpi_gatherv(loctsenworkvar3d(1:nxloc,1:nyloc,k), recvcounts2d(iope+1), mpi_real4, tsenworkvar3d(:,:,k), recvcounts2d, displs2d,&
                        mpi_real4, 0,iocomms(mem_pe(nproc)) ,iret)
            enddo
            varstrname = 'sphum'
            call fv3lamfile%get_idfn(varstrname,file_id,fv3filename)
